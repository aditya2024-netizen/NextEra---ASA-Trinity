...
interface AppContextType {
  ...
  setContactPatientModal: (
    data: { patient: Patient; findings?: AnalyzerFindings } | null,
  ) => void;
  ...
  isJavaModalOpen: boolean;
  setIsJavaModalOpen: (open: boolean) => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [explainModalPatient, setExplainModalPatient] =
    useState<Patient | null>(null);
  const [actionModalPatient, setActionModalPatient] = useState<Patient | null>(
    null,
  );
  const [interventionModalPatient, setInterventionModalPatient] =
    useState<Patient | null>(null);
  const [reportModalPatient, setReportModalPatient] = useState<Patient | null>(
    null,
  );

  // Admin Core Flow Modals
  const [editPatientModal, setEditPatientModal] = useState<Patient | null>(
    null,
  );
  const [contactPatientModal, setContactPatientModal] = useState<{
    patient: Patient;
    findings?: AnalyzerFindings;
  } | null>(null);
  const [analyzerFindingsModal, setAnalyzerFindingsModal] =
    useState<AnalyzerFindings | null>(null);
  ...
  const startDemoTour = () => setDemoTourStep(1);
  const nextDemoTourStep = () => {
    setDemoTourStep((step) => (step === null || step >= 5 ? null : step + 1));
  };
  const endDemoTour = () => setDemoTourStep(null);
  ...
  const [toasts, setToasts] = useState<ToastMessage[]>([
    {
      ...
      message:
        "Secure outpatient clinical follow-up intelligence system ready.",
    },
  ]);
  ...
  const addToast = (
    ...
    setToasts((prev) => [
      { id, type: finalType, title: finalTitle, message: finalMessage },
      ...prev.slice(0, 4),
    ]);
    setTimeout(() => {
  ...
  const updateScoringConfig = async (
    newConfig: ScoringConfiguration,
  ): Promise<boolean> => {
    try {
      ...
      if (res.success) {
        ...
        addToast(
          ...
          "Scoring weights saved and all risk ranks recalculated.",
        );
        ...
        addToast(
          "error",
          "Update Failed",
          (res as any).message || "Failed to update scoring configuration.",
        );
      }
    } catch (err: any) {
      addToast(
        "error",
        "Update Failed",
        err?.message || "Failed to update scoring configuration.",
      );
    }
    return false;
  };
  ...
  const runAnalyzerForPatient = async (
    patient: Patient,
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        addToast(
          "success",
          "Analyzer Findings Ready",
          `Generated deep clinical findings for ${patient.name} (Risk: ${res.data.riskScore}/100).`,
        );
        return res.data;
      } else {
        addToast(
          ...
          "Analyzer Error",
          (res as any).message ||
            "Could not process patient details in analyzer.",
        );
      }
    } catch (err: any) {
      addToast(
        "error",
        "Analyzer Error",
        err?.message || "Could not process patient details in analyzer.",
      );
    } finally {
    ...
    try {
      ...
      if (res.success) {
        ...
        addToast(
          "success",
          "Demo Reset",
          "Reseeded 1,000 synthetic patient records and canonical demo cases.",
        );
      } else {
        addToast(
          "error",
          "Reset Failed",
          res.message || "Could not reset demo database.",
        );
      }
    } catch (e: any) {
      addToast(
        "error",
        "Reset Failed",
        e?.message || "Could not reset demo database.",
      );
    }
  };
