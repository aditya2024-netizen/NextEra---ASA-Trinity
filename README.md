...
2. **Explain:** Transparent factor-by-factor point attribution answering _"Why is this patient at risk?"_ with top drivers and protective factors.
...
│  • Single Source of Truth — Persistent Database Storage      │
└─────────────────────────────────────────────────────────────┘
```
...
The application will launch at **`http://localhost:3000`**. PostgreSQL automatically provisions schemas and seeds 1,000 outpatient records on first boot. For local development without `DATABASE_URL`, the existing embedded PostgreSQL fallback is used.

### 5. Render Deployment

Create a Blueprint deployment in Render from this repository. Render reads `render.yaml`, creates the web service and managed PostgreSQL database, builds with `npm ci && npm run build`, and starts with `npm start`. Keep the generated `DATABASE_URL` and `JWT_SECRET` environment variables configured for the web service. The web service serves both the React application and `/api/*` endpoints on the same origin.

The production database is required: the application fails startup when `NODE_ENV=production` and `DATABASE_URL` is missing. `GEMINI_API_KEY` and the Twilio variables are optional; without them, the existing deterministic assistant and notification demo modes remain active.
...
_Quick 1-Click login buttons are available on the Login screen for instant testing._
...
6. **AI Assistant**: Open the clinical assistant (bottom-right) and ask _"Why is P-1042 high risk?"_ to review instant clinical driver attribution.
